 ...
 Tuple<double, double> FindIntersectionsOfLines(double k1, double b1, double k2, double b2)
{
    ...
    double y = (k2*b1-k1*b2)/(k2-k1);   //На мой взгляд логичнее было решить задачу без метода, он уж слишком узкопрофильный получается, но когда упёрся в то что не могу вывести две переменные из метода, захотелось разобратся, как это будет возможно реализовать. Вроде получилось ! )
